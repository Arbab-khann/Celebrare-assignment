...
function selectCard(cardHeading, selectedcard) {
  // rangebar
  let rangeScroll = document.createElement("input");
  rangeScroll.type = "range";
  rangeScroll.value = "10";
  rangeScroll.className = "rangeScroll";
  // changing the header
  header[1].style.display = "flex";
  header[1].appendChild(rangeScroll);
  ...
  // range slider
  rangeScroll.oninput = changeFontSize;
  function changeFontSize(newTaskDescription) {
    document.getElementById("newTaskDescription");
    newTaskDescription.style.fontSize = this.value + "px";
  }
}
